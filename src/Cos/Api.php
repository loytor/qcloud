...
class Api
{
    ...
    public function upload(
        $bucket, $file, $dstPath, $bizAttr = null, $sliceSize = null, $insertOnly = null)
    {

        $dstPath = $this->normalizerPath($dstPath, false);

        //文件大于20M则使用分片传输
        if ($file['size'] < self::MAX_UNSLICE_FILE_SIZE) {
            return $this->uploadFile($bucket, $file, $dstPath, $bizAttr, $insertOnly);
        } else {
            return array(
                'code' => -1,
                'message' => '文件大于20M，不能上传',
                'data' => array()
            );
            #$sliceSize = $this->getSliceSize($sliceSize);
            #return $this->uploadBySlicing($bucket, $file, $dstPath, $bizAttr, $sliceSize, $insertOnly);
        }
    }

    public function upload_bak(
     ...
     * @return [type]               [description]
     */
    private function uploadFile($bucket, $file, $dstPath, $bizAttr = null, $insertOnly = null)
    {
    ...
    private function uploadFile_Bak($bucket, $srcPath, $dstPath, $bizAttr = null, $insertOnly = null)
    {
    ...
    private function uploadBySlicing_bak(
        $bucket, $srcFpath, $dstFpath, $bizAttr = null, $sliceSize = null, $insertOnly = null)
    {
        $srcFpath = realpath($srcFpath);
        $fileSize = filesize($srcFpath);
